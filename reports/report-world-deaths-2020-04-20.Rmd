---
title: "Coronavirus Deaths in the World (April 20)"
...
data %>%
  ...
  subset(date < as.Date("2020-04-18")) ->
data
```
...
#### Coronavirus deaths per million inhabitants on March 20, 2020

We list the countries with the most coronavirus cases per million inhabitants. Not listed are China, San Marino, Andorra, Luxembourg, Monaco, Antigua and Barbuda, Iceland, Liechtenstein and all countries that had less than one death per million at the time (according to the available data).


```{r, echo=FALSE}
data %>%
  inner_join(slice(countries_sorted,1,2,11,14,20,23,24), by = c("country" = "country")) %>%
  ggplot() + geom_line(mapping = aes(x = date, y = per_million, color = country)) + labs(x="",y=paste(what,"per million"))
```

```{r, echo=FALSE}
data %>%
  inner_join(slice(countries_sorted,3:8), by = c("country" = "country")) %>%
  ggplot() + geom_line(mapping = aes(x = date, y = per_million, color = country)) + labs(x="",y=paste(what,"per million"))
```
...
```{r, echo=FALSE}
data %>%
  inner_join(slice(countries_sorted,9,10,12,13,15,16), by = c("country" = "country")) %>%
  ggplot() + geom_line(mapping = aes(x = date, y = per_million, color = country)) + labs(x="",y=paste(what,"per million"))
```

```{r, echo=FALSE}
data %>%
  inner_join(slice(countries_sorted,17:19,21:22,25), by = c("country" = "country")) %>%
  ggplot() + geom_line(mapping = aes(x = date, y = per_million, color = country)) + labs(x="",y=paste(what,"per million"))
```

```{r, echo=FALSE}
data %>%
  inner_join(slice(countries_sorted,26:35), by = c("country" = "country")) %>%
  ggplot() + geom_line(mapping = aes(x = date, y = per_million, color = country)) + labs(x="",y=paste(what,"per million"))
```

```{r, echo=FALSE}
data %>%
  inner_join(slice(countries_sorted, 36:45), by = c("country" = "country")) %>%
  ggplot() + geom_line(mapping = aes(x = date, y = per_million, color = country)) + labs(x="",y=paste(what,"per million"))
```
...
```{r, echo=FALSE}
data %>%
  inner_join(slice(countries_sorted, 58:106), by = c("country" = "country")) %>%
  ggplot() + geom_line(mapping = aes(x = date, y = per_million, color = country)) + labs(x="",y=paste(what,"per million"))
```
