---
title: "Coronavirus Cases in the World (April 20)"
...
data %>%
  ...
  subset(date < as.Date("2020-04-20")) ->
data
```
...
```{r, echo=FALSE}
data %>%
  inner_join(slice(countries_sorted,1,2,3,4,6), by = c("country" = "country")) %>%
...
```{r, echo=FALSE}
data %>%
  inner_join(slice(countries_sorted,13:25), by = c("country" = "country")) %>%
  ggplot() + geom_line(mapping = aes(x = date, y = per_million, color = country)) + labs(x="",y=paste(what,"per million"))
```
...
```{r, echo=FALSE}
data %>%
  inner_join(slice(countries_sorted,80:118), by = c("country" = "country")) %>%
  ggplot() + geom_line(mapping = aes(x = date, y = per_million, color = country)) + labs(x="",y=paste(what,"per million"))
```
