---
title: "Coronavirus Deaths in the World (March 20)"
...
We list the countries with the most coronavirus cases per million inhabitants. Not listed are China, San Marino, Iceland, Luxembourg and all countries that had less than one death per million at the time (according to the available data).

```{r, echo=FALSE}
#data %>%
#  inner_join(slice(countries_sorted,1,4,5), by = c("country" = "country")) %>%
#  ggplot() + geom_line(mapping = aes(x = date, y = per_million, color = country)) + labs(x="",y=paste(what,"per million"))
```

```{r, echo=FALSE}
data %>%
  inner_join(slice(countries_sorted,2,3,6,7,8,9), by = c("country" = "country")) %>%
  ggplot() + geom_line(mapping = aes(x = date, y = per_million, color = country)) + labs(x="",y=paste(what,"per million"))
```
...
```{r, echo=FALSE}
data %>%
  inner_join(slice(countries_sorted,10:23), by = c("country" = "country")) %>%
  ggplot() + geom_line(mapping = aes(x = date, y = per_million, color = country)) + labs(x="",y=paste(what,"per million"))
```
