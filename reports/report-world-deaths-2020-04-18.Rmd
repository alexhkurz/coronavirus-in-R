---
title: "Coronavirus Deaths in the World (May 6)"
...
#### Coronavirus deaths per million inhabitants on May 6, 2020

We list the countries with the most coronavirus cases per million inhabitants. Not listed are China, San Marino, Andorra, Luxembourg, Monaco, Antigua and Barbuda, Iceland, Liechtenstein and all countries that had less than 4 death per million at the time (according to the available data).
...
```{r, echo=FALSE}
data %>%
  inner_join(slice(countries_sorted,16,17,18,19,21,22), by = c("country" = "country")) %>%
  ggplot() + geom_line(mapping = aes(x = date, y = per_million, color = country)) + labs(x="",y=paste(what,"per million"))
```

```{r, echo=FALSE}
data %>%
  inner_join(slice(countries_sorted,25,26,27,28,29,30), by = c("country" = "country")) %>%
  ggplot() + geom_line(mapping = aes(x = date, y = per_million, color = country)) + labs(x="",y=paste(what,"per million"))
```

```{r, echo=FALSE}
data %>%
  inner_join(slice(countries_sorted,31:40), by = c("country" = "country")) %>%
  ggplot() + geom_line(mapping = aes(x = date, y = per_million, color = country)) + labs(x="",y=paste(what,"per million"))
```

```{r, echo=FALSE}
data %>%
  inner_join(slice(countries_sorted, 41:50), by = c("country" = "country")) %>%
  ggplot() + geom_line(mapping = aes(x = date, y = per_million, color = country)) + labs(x="",y=paste(what,"per million"))
```

```{r, echo=FALSE}
data %>%
  inner_join(slice(countries_sorted, 51:65), by = c("country" = "country")) %>%
  ggplot() + geom_line(mapping = aes(x = date, y = per_million, color = country)) + labs(x="",y=paste(what,"per million"))
```
